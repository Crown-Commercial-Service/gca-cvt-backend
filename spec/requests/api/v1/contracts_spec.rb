...
RSpec.describe 'GET /api/v1/contracts' do
  ...
  context 'when the caller is authenticated' do
    ...
    context 'organisation scoping' do
      let!(:other_org_contract) do
        create(:contract, title: 'Other Org Contract', ocid: 'ocds-other-org-001', organisation_id: 'org-2')
      end

      it 'excludes another organisation\'s contract from the results' do
        get '/api/v1/contracts'

        body = JSON.parse(response.body, symbolize_names: true)
        expect(body[:data].map { |c| c[:title] }).not_to include('Other Org Contract')
      end

      it 'excludes another organisation\'s contract from pagination totals' do
        get '/api/v1/contracts'
