...
RSpec.describe 'GET /api/v1/savings/:ocid/peer-comparison' do
  context 'when the caller is authenticated' do
    ...
    context 'when the contract belongs to a different organisation' do
      let!(:other_org_target) { create(:contract, :completed, amount: 100_000, organisation_id: 'org-2') }

      it 'returns 404, the same as a genuinely nonexistent OCID' do
        get "/api/v1/savings/#{other_org_target.ocid}/peer-comparison"
