...
RSpec.describe "api/v1/savings", type: :request do
  ...
  path "/api/v1/savings/{ocid}" do
    ...
    put "Update savings for an OCID" do
      tags "Savings"
      ...
      parameter name: :body, in: :body, required: true, schema: {
        type: :object,
        description: "Per-type arrays of savings updates. Each item must include a savings_id; only fields listed for that type are persisted. Optional calculation_completed flips the latest contract row at the end of the journey.",
        properties: {
          calculation_completed: { type: :boolean },
          cashable_savings: {
            type: :array,
      ...
      response "200", "savings updated; response mirrors GET /api/v1/savings/{ocid}" do
        schema(**success_schema)
      ...
      response "404", "no contract exists for the OCID, or a referenced savings_id is missing or belongs to a different OCID" do
        schema(**error_schema)

        let(:ocid) { "ocds-does-not-exist" }
        let(:body) { { calculation_completed: true } }

        run_test!
      end

      response "422", "a savings update item omitted savings_id, or persistence failed" do
        schema(**error_schema)

        let(:contract) { create(:contract) }
        let(:ocid) { contract.ocid }
        let(:body) { { cashable_savings: [ { baseline_value: 1 } ] } }
